  ...
  def update(self, CC, CS, now_nanos, frogpilot_toggles):
    ...
    if raw_regen_active:
      ...
      if not self.spoof_mid_sent and interval_ns > 0:
        ...
        if now_nanos >= midpoint_ns and now_nanos - self.last_steer_ts_ns >= 11_000_000:
      ...
      if self.spoof_accum >= 1.0 and not self.spoof_over_sent and interval_ns > 0:
        ...
        if now_nanos >= slot2_ns and now_nanos - self.last_steer_ts_ns >= 11_000_000:
          paddle_sends.append(gmcan.create_prndl2_command(self.packer_pt, CanBus.POWERTRAIN, True))
          paddle_sends.append(gmcan.create_regen_paddle_command(self.packer_pt, CanBus.POWERTRAIN, True))
          self.last_spoof_ts_ns = now_nanos
    ...
    if hasattr(self, "off_schedule_ns"):
      for i, t_ns in enumerate(self.off_schedule_ns):
        if not self.off_sent[i] and now_nanos >= t_ns and now_nanos - self.last_steer_ts_ns >= 11_000_000:
    ...
    if paddle_sends:
      # wait at least 11 ms after the last bus0 steer send
      if now_nanos - self.last_steer_ts_ns >= 11_000_000:
