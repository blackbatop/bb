...
void FrogPilotOnroadWindow::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  showFPS = fs.frogpilot_scene.started_timer >= 1 && frogpilot_toggles.value("show_fps").toBool();
...
void FrogPilotOnroadWindow::paintEvent(QPaintEvent *event) {
  ...
  if (showBlindspot || showSignal) {
    paintTurnSignalBorder(p, rect, sm);
  }
}

void FrogPilotOnroadWindow::paintFPS(QPainter &p, const QRect &rect, const FrogPilotUIState &fs) {
  ...
  avgFPS = totalFPS / fs.frogpilot_scene.started_timer;
...
void FrogPilotOnroadWindow::paintTurnSignalBorder(QPainter &p, const QRect &rect, const SubMaster &sm) {
  static bool leftFlickerActive = false;
  static bool rightFlickerActive = false;

  std::function<QColor(bool, bool, bool&)> getBorderColor = [&](bool blindSpot, bool turnSignal, bool &flickerActive) {
